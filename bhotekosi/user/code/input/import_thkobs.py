...
def import_thkobs(cfg, ds, ds_vars, path_data, RGI_version):
    ...
    rgi_outline.geometry = shapely.force_2d(rgi_outline.geometry)
    ...
    df_constrain = df[df["profile_id"].str.strip().str[-1].isin(cfg.inputs.oggm_shop.custom_thkobs_constrain)]
    df_test = df[df["profile_id"].str.strip().str[-1].isin(cfg.inputs.oggm_shop.custom_thkobs_test)]
    
    if df_constrain.empty:
        print("No profiles for constraint")
        ds_vars["thkobs"] = xr.full_like(ds_vars["thkinit"],np.nan)
        ds_vars["thkobs_std"] = xr.full_like(ds_vars["thkinit"],np.nan)
        ds_vars["thkobs_count"] = xr.full_like(ds_vars["thkinit"],np.nan)
    else:
